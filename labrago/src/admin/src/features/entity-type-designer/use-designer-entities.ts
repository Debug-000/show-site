...
export const useEntitiesDesigner = () => {
    ...
    const saveChanges = useCallback((fullEntity: ChangedFullEntity) => {
        ...
        client.mutate({ mutation: gql(query.query), variables: query.variables, fetchPolicy: "network-only" })
            .then((response) => {

                // const mutatedEntities = Object.keys(response.data).map(key => ({
    ...
    // const cleanUpChangesAndPrepareForNextCodeGeneration = useCallback((mutatedEntities: NameCaptionEntity[]) => {

    //     backendCodeGenerationEntitiesVar(mutatedEntities);
    //     designerChanges.revertAllChanges();
    // }, [designerChanges.revertAllChanges]);
